...
> A native management layer, persistent scheduling engine, and web-based GUI for the ClamAV Docker ecosystem.

## System Overview

ClamAV Scheduler abstracts the underlying ClamAV daemon, providing a persistent SQLite-backed task runner and a real-time diagnostic interface. Designed for strict environment segregation, it relies on direct host-to-container timezone mapping and dynamic log routing to maintain operational integrity without polluting the host environment.
...
* **Frontend Interface:** Native HTML/CSS/JS (Zero-dependency UI)
* **Scanner Daemon:** ClamAV (Cisco Talos)
* **Database:** SQLite (State and configuration persistence)
...
[ Host OS ] <===( Mount )===> [ Scanner Container ] <===( API )===> [ Backend Container ]
    |                               |                                      |
  Target                         clamd / freshclam                     FastAPI / UI
2. Prerequisites
...
Navigate to http://localhost:<HOST_UI_PORT> in a web browser.
...
Change LOG_LEVEL=DEBUG in your .env file.

Restart the backend container: docker compose restart antivirus-gui

Follow the trace: docker logs -f antivirus-gui
